...
export function NotificationPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 top-full mt-2 w-80 md:w-96 rounded-2xl overflow-hidden shadow-2xl shadow-black/60 z-50 border border-white/10"
      style={{ background: "rgba(10, 10, 10, 0.95)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)" }}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-white/10" style={{ background: "rgba(255,255,255,0.03)" }}>
          ...
          notifications.map((notif) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-semibold ${!notif.read ? "text-white" : "text-gray-300"}`}>
                  {notif.title}
                </p>
